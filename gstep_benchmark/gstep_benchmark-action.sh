...
function make_single_script()
{
    ...
    do
        ...
        if [[  $weekly != 'true' ]];then
            newline="LD_PRELOAD=/root/modelzoo/libjemalloc.so.2.5.1 $command $paras --no_eval --steps 6000 $bf16_para --checkpoint $checkpoint_dir$currentTime/${model_name,,}_$catg$log_tag  >$log_dir$currentTime/${model_name,,}_$catg$log_tag.log 2>&1"
        else
